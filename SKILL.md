...
The examples below are self-contained summaries of the main kernels and how a
typical host harness drives them. The host is the scalar driver that allocates
buffers, initializes inputs, calls the RVV kernel, and reads perf counters. The
kernel is the RVV intrinsics code that runs on the same core but uses vector
instructions (device-style computation inside a host-driven loop).
